...
RecoveryClientImpl::RecoveryClientImpl() noexcept : ringBuffer_{}, requests_{} {
    ringBuffer_.initialize();
}   

score::concurrency::InterruptibleFuture<void> RecoveryClientImpl::sendRecoveryRequest(
...
void RecoveryClientImpl::setResponseSuccess(std::size_t promise_id) noexcept {
    requests_[promise_id].promise_.SetValue();
    requests_[promise_id].in_use_ = false;
}

void RecoveryClientImpl::setResponseError(std::size_t promise_id, score::lcm::ExecErrc errType) noexcept {
    requests_[promise_id].promise_.SetError(errType);
    requests_[promise_id].in_use_ = false;
}

RecoveryRequest* RecoveryClientImpl::getNextRequest() noexcept {
