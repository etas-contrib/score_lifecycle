...
#include "platform/aas/mw/lifecycle/lifecyclemanager.h"
...
bmw::mw::lifecycle::LifeCycleManager::LifeCycleManager(std::unique_ptr<bmw::os::Signal> signal_interface) noexcept
    : m_stop_source{}, m_app{nullptr}, m_signal_set{}, m_signal_handler_thread{}, signal_(std::move(signal_interface))
{
    ...
    if (!initialize_internal())
    {
        ...
        bmw::os::Stdlib::instance().exit(EXIT_FAILURE);
        /* KW_SUPPRESS_END:MISRA.STDLIB.ABORT,MISRA.USE.EXPANSION */
    }
}

bmw::mw::lifecycle::LifeCycleManager::~LifeCycleManager() noexcept
{
    at_exit();
}

void bmw::mw::lifecycle::LifeCycleManager::at_exit() noexcept
{
    if (m_signal_handler_thread.joinable())
    {
        const auto result =
            signal_->SendSelfSigterm(); /* NOLINT(bmw-banned-function) using SendSelfSigterm by desing */
...
std::int32_t bmw::mw::lifecycle::LifeCycleManager::run(Application& app, const ApplicationContext& context)
{
    m_app = &app;
...
bool bmw::mw::lifecycle::LifeCycleManager::initialize_internal()
{
    ...
    const auto pthread_sig_mask_status =
        signal_->PthreadSigMask(m_signal_set); /* NOLINT(bmw-banned-function) using PthreadSigMask by desing */
...
void bmw::mw::lifecycle::LifeCycleManager::handle_signal()
{
    std::int32_t sig(0);
    ...
    else
    {
         ...
         * implementation defined. */
        bmw::os::Stdlib::instance().quick_exit(EXIT_FAILURE);
        /* KW_SUPPRESS_END:MISRA.STDLIB.ABORT,MISRA.USE.EXPANSION */
    }
}
